---
title: Search Auxiliary Files
description: "Reference for the auxiliary blob storage files backing the NuGetGallery search subsystem — download counts, ownership data, verified packages, popularity transfers, and excluded packages."
icon: "file-code"
---
...
Aside from metadata stored in the [Azure Search indexes](Azure-Search-indexes.mdx), there is data stored in Azure Blob
...
the [Search index](Azure-Search-indexes.mdx#search-index). Package IDs mentioned in the excluded packages list do appear
