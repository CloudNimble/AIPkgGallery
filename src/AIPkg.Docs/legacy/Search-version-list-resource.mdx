---
title: Search Version List Resource
description: "Reference for the blob-backed version list resource — the per-package-ID mapping of versions and their properties used by Db2AzureSearch and Catalog2AzureSearch to determine which search index documents to update."
icon: "list"
---
...
The [search index](./Azure-Search-indexes.mdx#search-index) stores up to 4 documents per package ID. These documents store the metadata for the latest listed version across these pivots:
...
Version lists are stored in the Azure Blob Storage container for [search auxiliary files](Search-auxiliary-files.mdx), in the `version-lists` folder. There is a JSON blob for each package ID where the blob's name is the package ID, lower-cased.
