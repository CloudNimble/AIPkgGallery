...
<Note>
  The manifest must be named exactly `{id}.aispec` where `{id}` matches the `id` field inside the manifest, be valid UTF-8 with no BOM, and conform to the schema defined in [spec 03](03-metadata-schema).
</Note>
...
<Note>
  Valid moniker directory names are the canonical APM names defined in [spec 02](02-platform-targeting). Unknown monikers are silently ignored by installers.
</Note>
...
<Info>
  Package signing is reserved for a future spec revision. When implemented, a detached signature will be stored at `.signature.p7s`, covering the full archive minus the signature entry itself. The registry will enforce signature validation for packages from verified authors.
</Info>
...
<Tabs>
  <Tab title="aipkg pack">
    ...
    See [spec 06](06-sdk-tooling) for full CLI details.
  </Tab>
  <Tab title="aipkg install">
    ...
    <Steps>
      ...
      <Step title="Resolve fallback chain">
        Resolves the fallback chain for the APM. See [spec 02](02-platform-targeting).
      </Step>
