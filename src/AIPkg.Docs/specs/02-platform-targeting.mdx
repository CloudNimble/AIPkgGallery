...
sidebarTitle: "02 · Platform Targeting"
...
| Moniker | Display Name | Parent | Install Path |
|---------|--------------|--------|--------------|
| `ai` | All AI Platforms | — | _(root fallback only)_ |
| `claude` | Claude (all) | `ai` | _(parent group only)_ |
| `claude-code` | Claude Code | `claude` | `.claude/` |
| `claude-desktop` | Claude Desktop | `claude` | See below |
| `copilot` | GitHub Copilot (all) | `ai` | _(parent group only)_ |
| `copilot-vscode` | Copilot in VS Code | `copilot` | `.github/` |
| `copilot-github` | Copilot on GitHub.com | `copilot` | `.github/` |
...
<Note>
  Compatibility checking is performed by `PlatformCompatibilityService` in `AIpkg.Core`. See [spec 06](06-sdk-tooling) for the API surface.
</Note>
...
The V3 search endpoint accepts an `apm` query parameter that filters results to packages compatible with the specified platform:
...
<Note>
  Stable monikers will not be removed or renamed. If a platform is discontinued, its moniker is deprecated (marked in the registry) but not deleted.
</Note>
