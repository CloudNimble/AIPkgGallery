---
title: "SDK & Tooling"
sidebarTitle: "06 · SDK & Tooling"
description: "`AIpkg.Core` library public API, `aipkg` CLI command reference, configuration file formats, and the post-MVP TypeScript SDK strategy."
icon: "terminal"
---

The AIpkg SDK consists of three components:

<CardGroup cols={3}>
  <Card title="AIpkg.Core" icon="cube">
    Cross-platform .NET 10 class library. Consumed by both the server and CLI.
  </Card>
  <Card title="aipkg CLI" icon="terminal">
    .NET 10 Native AOT single-file binary. No runtime required.
  </Card>
  <Card title="TypeScript SDK" icon="code">
    `@aipkg/core` and platform packages. Post-MVP.
  </Card>
</CardGroup>
...
`AIpkg.Core` is the shared parsing, validation, and archive-handling library. It has no dependency on ASP.NET Core or any server infrastructure. Both `AIpkg.Server` and `AIpkg.CLI` reference it.
...
<Note>
  Documentation coverage requirement: 100% of public surface members must carry XML doc comments before the 1.0.0 release. See [spec 07](07-docs-platform) for DotNetDocs.com integration.
</Note>
...
| Windows | `winget install aipkg` |
| macOS | `brew install aipkg` |
| Linux (future) | `apt install aipkg` |
| Script | `curl -fsSL https://aipkg.org/install.sh \| sh` |

Ships for: `win-x64`, `linux-x64`, `linux-arm64`, `osx-x64`, `osx-arm64`. No .NET runtime required.

Published with: `dotnet publish -r {rid} --self-contained`
...
<AccordionGroup>
  ...
  <Accordion title="aipkg verify">
    ...
    Validates:
      ...
      - Manifest business rules (see spec 03)
...
## Part 3: TypeScript SDK (Post-MVP)
